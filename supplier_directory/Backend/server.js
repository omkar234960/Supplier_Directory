...
// ✅ Proper CORS setup for your Netlify frontend
app.use(
  cors({
    origin: ["https://supplier-directory.netlify.app"], // your Netlify domain
...
app.post("/api/suppliers", async (req, res) => {
  try {
    const { name, category, email, phone, whatsapp, location, rating = 4.0, products = "", notes = "" } = req.body;
...
app.post("/api/suppliers/:id/contact", async (req, res) => {
  try {
    ...
    await runAsync("INSERT INTO messages (supplier_id, method, message) VALUES (?, ?, ?)", [id, method, message]);
